...
for (let i = 1; i <= 15; i++) {
    ...
    let edad = parseInt(prompt(`Comprador ${i}: ¿Cuál es su edad?`));

    // Pido al usuario si la película es para adultos
    let tipo = prompt("¿La película es para adultos? (sí/no)");

    // Verifico si el comprador es menor de edad y la película es para adultos
    if (edad < 18 && tipo === "sí") {

        // Si se cumple la condición, rechazo la venta
        console.log(`Venta rechazada para comprador ${i}: menor de edad para película para adultos`);
